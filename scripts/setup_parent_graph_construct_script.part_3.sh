...
Inputs:
    ...
    -t PATH to .ped file containing only the mother-father-proband trio samples
...
while getopts "p:m:f:t:w:g:v:h" OPTION; do
    case $OPTION in
        p)
...
echo "source /data/${USER}/vg_wdl_testrun/miniwdl_venv/bin/activate" >> ${COHORT_WORKFLOW_DIR}/${PROBAND_SAMPLE_NAME}_cohort_parental_graph_construction.part_3.sh
